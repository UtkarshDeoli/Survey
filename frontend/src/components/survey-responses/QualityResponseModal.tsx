...
function QualityResponseModal({
  ...
  const remarksToShow =
  selectedResponse?.quality_check_remarks && selectedResponse.quality_check_remarks.length > 0
    ? showAll
      ? [...selectedResponse.quality_check_remarks].reverse()
      : [...selectedResponse.quality_check_remarks].reverse().slice(0, 2)
    : [];
            ...
            <div className="flex h-full w-full flex-col overflow-y-auto vertical-scrollbar">
              ...
              <div className="grid grid-cols-2 w-full">
                ...
                <div className="grid grid-cols-2 py-4 bg-primary-100 w-full text-center h-full font-medium">
                  {selectedResponse.quality_check_remarks &&
                  selectedResponse.quality_check_remarks.length > 0 ? (
                    <>
                      <p className="font-semibold">
                        Quality remark note:
                      </p>
                      <p className="font-normal ml-2">
                        {selectedResponse.quality_check_remarks[selectedResponse.quality_check_remarks.length-1].note}
                      </p>
                      <p className="font-semibold">
                        Date:{" "}
                      </p>
                      <p className="font-normal ml-2">
                        {formatDate(selectedResponse.quality_check_remarks[selectedResponse.quality_check_remarks.length-1].date)}
                      </p>
                      <p className="font-semibold">
                        Time:{" "}
                      </p>
                      <p className="font-normal ml-2">
                        {formatTime(selectedResponse.quality_check_remarks[selectedResponse.quality_check_remarks.length-1].date)}
                      </p>
                    </>
                  ) : (
                    "---"
              ...
              <div className="grid grid-cols-2 w-full bg-primary-50">
                <h2 className="font-medium py-4 text-center">Quality remark history</h2>
                {selectedResponse.quality_check_remarks &&
                  selectedResponse.quality_check_remarks.length > 0? (
                  <div className="p-4 flex flex-col items-center ">
                    ...
                    {selectedResponse.quality_check_remarks.length > 2 && (
                      <button
...
export default QualityResponseModal;
