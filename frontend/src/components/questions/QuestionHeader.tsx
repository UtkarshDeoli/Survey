...
function QuestionHeader({id, created_by, name}:props) {
    ...
    <header className="sticky top-0 left-0 w-full h-16 border-2 z-20">
        <div className="bg-secondary-100 h-full w-full px-8 py-3 flex justify-between items-center">
           ...
           <div className=" flex gap-2">
                <button onClick={()=>router.push(`/admin/surveys/questions?id=${id}&created_by=${created_by}`)} className={`border text-black bg-primary-300 px-4 py-2 rounded-md text-[14px] font-sem font-semibold ${path.includes("/admin/surveys/questions") ? "text-white" : "bg-secondary-200"}`}>Questionnaire</button>
                <button onClick={()=>router.push(`/admin/surveys/resequence?id=${id}&created_by=${created_by}`)} className={`border text-black bg-primary-300 px-4 py-2 rounded-md text-[14px] font-sem font-semibold ${path.includes("/admin/surveys/resequence")  ? "text-white" : "bg-secondary-200"}`}>Resequence</button>
