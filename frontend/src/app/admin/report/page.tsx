...
function page() {
 ...
 console.log(data)
  ...
  return (
    <div className="w-full font-medium bg-[#ECF0FA] min-h-[calc(100vh-80px)]">
      <nav className="h-16 w-full py-3 px-8 flex justify-between">
        <div className="text-my-gray-200">
          <h1 className="text-2xl">Surveys Report</h1>
        </div>
      ...
      <div className="w-full px-5 py-5 text-sm">
        ...
        {!loading &&
          data.length > 0 ? data.map((el: any, index: number) => (
            <div
              onClick={()=>router.push(`/admin/report/survey-responses?survey_id=${el.survey_id}`)}
              key={index}
            ...
            >
              ...
              <p className="col-span-1 flex justify-center items-center">
                <BsPieChart
                  ...
                  onClick={(e) => {
                    e.stopPropagation()
                  router.push(`/admin/report/analytics?survey_id=${el.survey_id}`)
                  }}
