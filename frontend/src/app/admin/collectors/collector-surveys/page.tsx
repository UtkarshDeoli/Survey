    ...
    <div className="w-full bg-[#ECF0FA] text-sm min-h-[calc(100vh-80px)]">
      ...
      <nav className="h-16 w-full py-3 px-8 flex justify-between">
        <div className="text-my-gray-200">
          <h1 className="text-2xl font-medium">Collector's Surveys</h1>
        </div>
      </nav>

      {/* Surveys */}
      <div className="relative w-[96%] mx-auto text-sm mt-5  max-h-[60vh] overflow-y-auto vertical-scrollbar">
        <div className="sticky top-0 left-0 z-10 grid grid-cols-4 text-white bg-dark-gray font-semibold py-[16px] rounded-tl-2xl rounded-tr-2xl border border-secondary-200">
        ...
        {!loading && collectorSurveys && collectorSurveys.length !== 0 ? (
          collectorSurveys.map((survey: any, index: number) => (
            <div
              key={index}
              className="bg-mid-gray border-2 grid p-2 grid-cols-4 gap-2 text-center text-black"
            >
              ...
              <ButtonFilled onClick ={()=>router.push(`/admin/collectors/collector-surveys/data?survey_id=${survey._id}&collector_id=${collectorId}`)} className="w-fit mx-auto">
                View Responses
              </ButtonFilled>
            </div>
          ))
